...
class SzSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for unit in self.units:
            ...
            self.driver.find_element_by_id("headerKey").send_keys(unit.company_allname)
            ...
            try:
                ...
                for tab in tabs:
                    if tab != searching:
                        ...
                        qichachca.BaseInfo(response, unit.id)

                        qichachca.Unit_Base_Shareholder_Info(response, unit.id)

                        qichachca.Unit_Base_Changed_Info(response, unit.id)
                        ...
                        # aa = self.driver.page_source
                        # response = Selector(text=aa)
                        # qichachca.Unit_annual_reports_Base_Info(response, unit.id)
                    else:
                        self.driver.close()
                time.sleep(2)
