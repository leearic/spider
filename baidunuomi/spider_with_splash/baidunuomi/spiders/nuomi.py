    ...
    def parse(self, response):
        ...
        for index in zones:
            ...
            if isinstance(zone_text, list) and len(zone_text) != 0:
               yield Request(url=zone_link[0], callback=self.parse_zone,
                             meta={ 'zone':zone_text[0],
                                    'splash': {
    ...
    def parse_zone(self, response):
        ...
        for index in categorys:
            ...
            if "nuomi.com" not in category_link[0]:
                pass
                # print category_link[0]
            else:
                yield  Request(url=category_link[0], callback=self.parse_category,
                               meta={ 'zone':zone, 'category': category_text,
                                    'splash': {
                                        'endpoint' : "render.html",
                                        'args': {
                                            'wait': 0.5,
